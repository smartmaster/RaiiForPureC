...
namespace SmartLib
{
	...
	private:
		...
		void MakeFunction()
		{
			...
			assert(ExprType::ET_ID == funcId->GetExprType());


			assert(_argsStack.size());
			std::vector<upExpr<T>> args = PopTop(_argsStack);

			upExpr<T> funcExpr = std::make_unique<ExpressionFuncOrId<T>>(std::move(funcId), std::move(args));
		...
		void ProcessPossilbleLastArg()
		{
			...
			if (ExprType::ET_ID == current->GetExprType()) //f(		)
			{
				MakeFunction();
			}
			else //f(arg		)
			{
				...
				if (arg && funcid && ExprType::ET_ID == funcid->GetExprType())
				{
...
#else
			upExpr<T> argMaybe = _exprStack.size() ? PopTop(_exprStack) : nullptr;
			upExpr<T> funcIdMaybe = _exprStack.size() ? PopTop(_exprStack) : nullptr;
			//f(g )
			//f(3 )
			if (funcIdMaybe && ExprType::ET_ID == funcIdMaybe->GetExprType())
			{
				_exprStack.push(std::move(funcIdMaybe));
				_exprStack.push(std::move(argMaybe));
				AddToArgs();
				MakeFunction();
			}
			else if (argMaybe && ExprType::ET_ID == argMaybe->GetExprType()) //f( )
			{
				if (funcIdMaybe)
				{
					_exprStack.push(std::move(funcIdMaybe));
				...
				_exprStack.push(std::move(argMaybe)); //in fact argMaybe is function name (not a var)
